...
const ListsContainer = ({ lists, tasks, cardRefs, containerRef, showNewTaskModal, onDelete }) => {
    return (
        <div className="bg-[#D4EBF8] w-full h-full">
            ...
            <div
                ref={ containerRef }
                className="h-full py-5 px-5 flex gap-3 items-start overflow-x-scroll lg:pr-0 md:pr-[600px]"
            >
                {lists.map((listItem) => (
                    <ListCard
                        key={listItem.id}
